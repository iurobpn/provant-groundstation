	...
	def takeData(self):
		...
		if(self.who==MSP_RAW_GPS):
			...
			if(check==ord(self.L[self.size])):
				...
				self.raw_gps.lat    =ord(self.L[2])+(ord(self.L[3])<<8)+(ord(self.L[4])<<16)+(ord(self.L[5])<<24)
				self.raw_gps.lon    =ord(self.L[6])+(ord(self.L[7])<<8)+(ord(self.L[8])<<16)+(ord(self.L[9])<<24)
		...
		if(self.who==MSP_ALTITUDE):
			...
			for x in xrange(0,self.size):
				check^=ord(self.L[x])
			if(check==ord(self.L[self.size])):
				self.altitude.alt       =ord(self.L[0])+(ord(self.L[1])<<8)+(ord(self.L[2])<<16)+(ord(self.L[3])<<24)
		...
		if(self.who==MSP_STATUS):
			...
			if(check==ord(self.L[self.size])):
				...
				self.status.flag       =ord(self.L[6])+(ord(self.L[7])<<8)+(ord(self.L[8])<<16)+(ord(self.L[9])<<24)
		...
		if(self.who==MSP_IDENT):
			...
			if(check==ord(self.L[self.size])):
				...
				self.ident.capability   =ord(self.L[3])+(ord(self.L[4])<<8)+(ord(self.L[5])<<16)+(ord(self.L[6])<<24)
				'''
